/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   stack_1.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jsoh <jsoh@student.42.fr>                  +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/22 18:58:27 by jsoh              #+#    #+#             */
/*   Updated: 2025/09/06 17:25:28 by jsoh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pushswap.h"

void	stack_create(s_stack **stack, int size, char type)
{
	(*stack) = (s_stack *)malloc(sizeof(s_stack));
	if (!stack)
		return ;
	(*stack)->array = (int *)malloc(size * sizeof(int));
	if (!(*stack)->array)
		return ;
	(*stack)->test_array = (s_num *)malloc(size * sizeof(s_num));//test
	if (!(*stack)->test_array)
		return ;
	if (type == 'a')
		(*stack)->cnt = size - 1;
	else if (type == 'b')
		(*stack)->cnt = 0;
	(*stack)->cap = size;
	//printf("Stack created\n");
}

void	stack_init(s_stack *stack, char const *argv[])
{
	int	index;

	index = 0;
	ft_memset(stack->array, 0, sizeof(int) * stack -> cap);
	ft_memset(stack->test_array, 0, sizeof(stack->test_array[0]) * stack -> cap);
	while (index < stack->cnt)
	{
		stack -> array[index] = ft_atoi(argv[index + 1]);
		stack -> test_array[index].val = ft_atoi(argv[index + 1]);
		stack -> test_array[index].index = index;
		index++;
	}
}

void	stack_view(s_stack *stack)
{
	int	index;

	index = 0;
	printf("Stack View: %i\n",stack -> cnt);
	while (index < stack -> cnt)
	{
		printf("num: %-6i order: %-6i\n",stack -> test_array[index].val, stack -> test_array[index].order);
		index++;
	}
}

void	stack_free(s_stack **stack)
{
	if (!stack || !*stack)
		return ;
	free((*stack)->array);
	free(*stack);
	*stack = NULL;
}
