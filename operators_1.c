...
/*   Updated: 2025/08/24 16:42:04 by jsoh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	swap(s_stack *stk_a, s_stack *stk_b)
{
	int	top_a;

	if (!stk_a || !stk_b || stk_a->count == 0 || stk_b->count == 0)
		return ;
	top_a = stk_a->array[stk_a->start].num;
	stk_a->array[stk_a->start].num = stk_b->array[stk_b->start].num;
	stk_b->array[stk_b->start].num = top_a;
}
